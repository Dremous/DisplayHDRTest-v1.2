...
//	m_XRitePatchTimer = 0;						// v1.5 timer to run until DisplayTime is up.
	...
	m_testPatternResources[TestPattern::TenPercentPeak]    = TestPatternResources{ std::wstring(L"背景噪声")                          , std::wstring()                                , std::wstring(L"BackgroundNoiseEffect.cso")   , CLSID_CustomBackgroundNoiseEffect };
	m_testPatternResources[TestPattern::BitDepthPrecision] = TestPatternResources{ std::wstring(L"7. 位深度/精度")                    , std::wstring()                                , std::wstring(L"BandedGradientEffect.cso")    , CLSID_CustomBandedGradientEffect };
	m_testPatternResources[TestPattern::SharpeningFilter]  = TestPatternResources{ std::wstring(L"菲涅尔波带板（锐度测试）")      , std::wstring()                                , std::wstring(L"SineSweepEffect.cso")         , CLSID_CustomSineSweepEffect };
	m_testPatternResources[TestPattern::ToneMapSpike]      = TestPatternResources{ std::wstring(L"ST.2084 脉冲（色调映射测试）")             , std::wstring()                                , std::wstring(L"ToneSpikeEffect.cso")         , CLSID_CustomToneSpikeEffect };
	m_testPatternResources[TestPattern::OnePixelLinesBW]   = TestPatternResources{ std::wstring(L"单像素线条（黑白）")          , std::wstring(L"OnePixelLinesBW1200x700.png")  , std::wstring()                               , {} };
    m_testPatternResources[TestPattern::OnePixelLinesRG]   = TestPatternResources{ std::wstring(L"单像素线条（红绿）")            , std::wstring(L"OnePixelLinesRG1200x700.png")  , std::wstring()                               , {} };
    m_testPatternResources[TestPattern::TextQuality]       = TestPatternResources{ std::wstring(L"抗锯齿文本（ClearType 和灰度）"), std::wstring(L"CalibriBoth96Dpi.png")         , std::wstring()                               , {} };

    m_hideTextString = std::wstring(L"按 SPACE 键隐藏此文本。");
...
WCHAR *Game::GetTierName(Game::TestingTier tier)
{
    ...
    else return L"不支持的 DisplayHDR 等级";
}
...
void Game::GenerateTestPattern_StartOfTest(ID2D1DeviceContext2* ctx)
{
	...
	text << L"→, PAGE DN:       切换到下一个测试\n";
	text << L"←, PAGE UP:        切换到上一个测试\n";
    text << L"数字键:	跳转到测试编号（Shift 键同时按下为 v1.2 测试）\n";
    text << L"SPACE:		隐藏文本和目标圆环\n";
    text << L"C:		开始 60 秒冷却\n";
    text << L"ALT-ENTER:	切换全屏\n";
    text << L"ESCAPE:		退出全屏\n";
    text << L"ALT-F4:		退出应用\n";
    text << L"\nCopyright � VESA\n最后更新：" << __DATE__;
    text << L"\n包含 Portrait X-Rite� 色彩技术\n";
...
void Game::GenerateTestPattern_ConnectionProperties(ID2D1DeviceContext2* ctx)
{
	...
	text << " x " << std::to_wstring(m_outputDesc.BitsPerColor) << L"位 @ ";
...
void Game::GenerateTestPattern_PanelCharacteristics(ID2D1DeviceContext2* ctx)
{
	...
	if (CheckForDefaults())
	{
		text << L"***警告：这些是操作系统提供的默认值。显示器未提供任何数据。***\n";
	}
	// else
	{
		text << L"\nHDR 测试等级：" << GetTierName(m_testingTier);
		text << L" - 使用上/下箭头键更改" << L"\n";
	}
...
//  text << std::to_wstring(m_outputDesc.MaxLuminance / m_outputDesc.MinLuminance );
	...
	text << L"\nCIE 1931          x         y";
    text << L"\n红色基色：  " << std::to_wstring(red_xy.x) << L"  " << std::to_wstring(red_xy.y);
    text << L"\n绿色基色：  " << std::to_wstring(grn_xy.x) << L"  " << std::to_wstring(grn_xy.y);
    text << L"\n蓝色基色：  " << std::to_wstring(blu_xy.x) << L"  " << std::to_wstring(blu_xy.y);
    text << L"\n白点：  " << std::to_wstring(wht_xy.x) << L"  " << std::to_wstring(wht_xy.y);
    ...
    text << L"\n   % 人眼色域：" << std::to_wstring(gamutAreaDevice / gamutAreaHuman*100.f);

    // test code:
    float theory = gamutAreaDevice / gamutAreaBT2100;
    //	text << L"\n     %  Theory : " << std::to_wstring( theory*100.f);
    //	text << L"\n       % 错误 : " << std::to_wstring((coverage2100 - theory) / theory * 100.f);

    RenderText(ctx, m_monospaceFormat.Get(), text.str(), m_largeTextRect);

    if (m_showExplanatoryText)
    {
        std::wstring title = L"报告的面板特性\n" + m_hideTextString;
        RenderText(ctx, m_largeFormat.Get(), title, m_testTitleRect);
...
// EDID settings don't update afterwards anyway until next app run
	...
	text << L"显示器应处于正常工作温度状态。\n";
    text << L"  -某些面板可能需要长达 30 分钟才能达到热平衡。\n";

    text << L"  -另请注意，环境温度可能会影响测试结果。\n\n";

    text << L"使用以下屏幕检查正确的亮度级别";
...
void Game::GenerateTestPattern_PQLevelsInNits(ID2D1DeviceContext2 * ctx)		// 
{
    ...
    if (m_showExplanatoryText)
    {
        ...
        std::wstring title = L"PQ/ST 2084 亮度级别（nits）\n" + m_hideTextString;
        RenderText(ctx, m_largeFormat.Get(), title, m_testTitleRect);
...
void Game::GenerateTestPattern_TenPercentPeak(ID2D1DeviceContext2* ctx) //********************** 1.a
{
	...
	{
		title << L"\nERROR: " << rsc.effectShaderFilename << L" 丢失\n";
	}
	else
	{
...
void Game::GenerateTestPattern_FlashTestMAX(ID2D1DeviceContext2 * ctx) //********************* 2.MAX
{
    ...
    if (m_flashOn)
    {
        title << "2.b MAX 闪烁测试 开\n";
        ctx->FillRectangle(m_deviceResources->GetLogicalSize(), peakFFWhite.Get());
    }
    else
    {
        title << "2.b MAX 闪烁测试 关\n";
        ctx->FillRectangle(m_deviceResources->GetLogicalSize(), blackBrush.Get());
    }
...
void Game::GenerateTestPattern_LongDurationWhite(ID2D1DeviceContext2 * ctx) //******************* 3.
{
    ...
    if (m_showExplanatoryText)
    {
        ...
        title << L"3.a 全帧白色 30 分钟：";
        if (0.0f != m_testTimeRemainingSec)
        {
            title << static_cast<unsigned int>(m_testTimeRemainingSec);
            title << L" 秒剩余\n通过渲染报告的 MaxFALL 值测试散热方案";
            title << L"\nNits: ";
            title << nits*BRIGHTNESS_SLIDER_FACTOR;
            title << L"  HDR10: ";
			title << setprecision(0);
...
void Game::GenerateTestPattern_FullFramePeak(ID2D1DeviceContext2 * ctx)	//******************** 3.MAX
{
    ...
    if (m_showExplanatoryText)
    {
        ...
        title << L"3.b 全帧白色 30 分钟：";
        if (0.0f != m_testTimeRemainingSec)
        {
            title << static_cast<unsigned int>(m_testTimeRemainingSec);
            title << L" 秒剩余";
            title << L"\n渲染 HDR10 最大值（10,000 nits）";
            title << L"\nNits: ";
            title << nits;
            title << L"  HDR10: ";
			title << setprecision(0);
...
void Game::GenerateTestPattern_ColorPatchesMAX(ID2D1DeviceContext2 * ctx, float OPR) // *******6.MAX
{
 ...
 // const float nits = 300.0f;								// white level should be 300 cd/m2
    ...
    title << L"6.b 检查 ";	// show test number
...
void Game::GenerateTestPattern_ColorPatches(ID2D1DeviceContext2 * ctx)
{
    ...
    RenderText(ctx, m_smallFormat.Get(), L"红：50% sRGB   ", { w * 0.0f + w1, h * 1.0f - h1 / 1.0f, 200.0f, 30.0f });
    RenderText(ctx, m_smallFormat.Get(), L"红：100% sRGB  ", { w * 1.0f + w1, h * 1.0f - h1 / 1.0f, 200.0f, 30.0f });
    RenderText(ctx, m_smallFormat.Get(), L"红：DCI-P3     ", { w * 2.0f + w1, h * 1.0f - h1 / 1.0f, 200.0f, 30.0f });
    //RenderText(ctx, m_smallFormat.Get(), L"Red: Rec. 2020  ", { w * 3.0f + w1, h * 1.0f - h1 / 1.0f, 200.0f, 30.0f });
    RenderText(ctx, m_smallFormat.Get(), L"绿：50% sRGB ", { w * 0.0f + w1, h * 2.0f - h1 / 1.0f, 200.0f, 30.0f });
    RenderText(ctx, m_smallFormat.Get(), L"绿：100% sRGB", { w * 1.0f + w1, h * 2.0f - h1 / 1.0f, 200.0f, 30.0f });
    RenderText(ctx, m_smallFormat.Get(), L"绿：DCI-P3   ", { w * 2.0f + w1, h * 2.0f - h1 / 1.0f, 200.0f, 30.0f });
    //RenderText(ctx, m_smallFormat.Get(), L"Green: Rec. 2020", { w * 3.0f + w1, h * 2.0f - h1 / 1.0f, 200.0f, 30.0f });
    RenderText(ctx, m_smallFormat.Get(), L"蓝：50% sRGB  ", { w * 0.0f + w1, h * 3.0f - h1 / 1.0f, 200.0f, 30.0f });
    RenderText(ctx, m_smallFormat.Get(), L"蓝：100% sRGB ", { w * 1.0f + w1, h * 3.0f - h1 / 1.0f, 200.0f, 30.0f });
    RenderText(ctx, m_smallFormat.Get(), L"蓝：DCI-P3    ", { w * 2.0f + w1, h * 3.0f - h1 / 1.0f, 200.0f, 30.0f });
    //RenderText(ctx, m_smallFormat.Get(), L"Blue: Rec. 2020 ", { w * 3.0f + w1, h * 3.0f - h1 / 1.0f, 200.0f, 30.0f });

    std::wstring text = L"RGB 色块\n" << m_hideTextString;
...
void Game::GenerateTestPattern_BitDepthPrecision(ID2D1DeviceContext2 * ctx) //******************* 7.
{
    ...
    if (!rsc.effectIsValid)
    {
        title << L"\nERROR: BandedGradientEffect.cso 丢失\n";
    }
    else
    {
    ...
    if (m_showExplanatoryText)
    {
        ...
        RenderText(ctx, m_largeFormat.Get(), L"6 位量化", text10Rect);
        RenderText(ctx, m_largeFormat.Get(), L"显示器原生", text30Rect);
        //	RenderText(ctx, m_largeFormat.Get(), L"6+2 dither 2x2",		text30Rect);
        RenderText(ctx, m_largeFormat.Get(), L"8 位量化", text50Rect);
        RenderText(ctx, m_largeFormat.Get(), L"显示器原生", text70Rect);
        RenderText(ctx, m_largeFormat.Get(), L"10 位量化", text90Rect);
...
void Game::GenerateTestPattern_LocalDimmingContrast(ID2D1DeviceContext2* ctx)				// v1.2.1
{
	...
	case 0:				// 单维 local dimming test image
	{
#ifdef NOISE
		// Draw the background noise effect
		auto rsc = m_testPatternResources[TestPattern::TenPercentPeak];
	...
	case 1:													// 二维 local dimming test image
	{
...
void Game::GenerateTestPattern_BlackLevelHDRvsSDR(ID2D1DeviceContext2* ctx)			   		// v1.2.2
{
	...
	if (m_showExplanatoryText)
	{
		...
		title << L"1.2.2 HDR 与 SDR 黑色级别对比";
		title << L"\nNits: ";
		title << nits * BRIGHTNESS_SLIDER_FACTOR;
		title << L"  HDR10: ";
		title << setprecision(0);
...
void Game::GenerateTestPattern_SubTitleFlicker(ID2D1DeviceContext2 * ctx)	        		// v1.2.4
{
	...
	if (m_subtitleVisible & 0x01)			// only switch visibility on last bit
	{
		...
		std::wstring subtitle = L"这是用于测试的字幕文本字符串";
...
//		title << L" adjust using Up/Down arrow keys";
		...
		title << L"  每 " << m_XRitePatchDisplayTime << "sec";
...
void Game::GenerateTestPattern_StaticGradient(ID2D1DeviceContext2* ctx)
{
    ...
    if (m_showExplanatoryText)
    {
        ...
        std::wstringstream text;
        text << L"静态渐变：R(" << DX::to_string_with_precision(m_gradientColor.r)
...
void Game::GenerateTestPattern_FullFrameSDRWhiteWithHDR(ID2D1DeviceContext2 * ctx)
{
    ...
    if (m_showExplanatoryText)
    {
        std::wstringstream title;
		title << fixed << setw(8) << setprecision(2);
        title << L"全帧 SDR 白色，带 10% HDR 窗口（" << nits;
...
void Game::GenerateTestPattern_ImageCommon(ID2D1DeviceContext2* ctx, TestPatternResources resources)
{
    ...
    if (m_showExplanatoryText)
    {
        ...
        {
            text << L"\nERROR: " << resources.imageFilename << " 丢失。";
        }
